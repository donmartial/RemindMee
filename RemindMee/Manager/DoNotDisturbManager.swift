...
class DoNotDisturbManager: ObservableObject {
    ...
    private func checkCurrentFocusStatus() {
        Task { @MainActor in
            ...
            isCurrentlyInFocus = (respectSystemFocus && systemFocusActive) || 
                               (respectCalendarEvents && calendarBusy)
            currentCalendarEvent = eventTitle
            
            // Log changes
            if wasInFocus != isCurrentlyInFocus {
                print("Focus status changed: \(isCurrentlyInFocus ? "Entered" : "Exited") focus mode")
    ...
    private func checkCalendarStatus() -> (Bool, String?) {
        ...
        for event in events {
            if event.startDate > now && event.startDate <= soonThreshold {
                // Event starting soon
                return (true, "Upcoming: \(String(describing: event.title))")
            }
        }
